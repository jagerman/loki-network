...
[
    ...
    // Static AMD64 and armhf builds (get uploaded)
    debian_pipeline("Static (buster ARM64)", "debian:buster", arch="arm64", deps='g++ python3-dev',
                    cmake_extra='-DBUILD_STATIC_DEPS=ON -DBUILD_SHARED_LIBS=OFF -DSTATIC_LINK=ON ' +
                        '-DCMAKE_CXX_FLAGS="-march=armv8-a" -DCMAKE_C_FLAGS="-march=armv8-a" -DNATIVE_BUILD=OFF ' +
                        '-DWITH_SYSTEMD=OFF',
                    extra_cmds=[
                        '../contrib/ci/drone-check-static-libs.sh',
                        'UPLOAD_OS=linux-aarch64 ../contrib/ci/drone-static-upload.sh'
                    ]),
